...
import './style.css';
import { getLinksForHost, NEON, NODE_LAYOUT } from './config.js';
  ...
  <div id="app-shell" class="app-shell relative min-h-screen overflow-hidden bg-slate-950 text-slate-100">
    ...
    <header class="pointer-events-none absolute inset-x-0 top-0 z-20 flex flex-col gap-4 px-4 pb-4 pt-4 md:px-8">
      <div class="pointer-events-auto flex flex-wrap items-start justify-between gap-4 rounded-2xl border border-cyan-500/20 bg-slate-950/70 px-5 py-4 shadow-[0_0_30px_rgba(34,211,238,0.12)] backdrop-blur-xl">
        ...
        <div class="utility-bar flex flex-wrap gap-2">
    ...
    <main class="relative z-10 min-h-screen">
      ...
      <aside class="pointer-events-none absolute bottom-4 left-4 z-20 w-[min(100%-2rem,24rem)] rounded-2xl border border-slate-800/80 bg-slate-950/75 p-4 shadow-[0_0_45px_rgba(14,165,233,0.12)] backdrop-blur-xl md:bottom-8 md:left-8">
        <div class="pointer-events-auto flex items-center justify-between gap-3">
          <div>
            ...
            <h2 id="selected-node-name" class="mt-2 text-2xl font-semibold text-white">Monitoring Core</h2>
      ...
      <section class="pointer-events-none absolute right-4 top-28 z-20 w-[min(100%-2rem,26rem)] rounded-2xl border border-slate-800/80 bg-slate-950/70 p-4 shadow-[0_0_40px_rgba(168,85,247,0.1)] backdrop-blur-xl md:right-8 md:top-32">
        <div class="pointer-events-auto flex items-center justify-between gap-3">
          <div>
        ...
        <div id="node-grid" class="mt-4 grid gap-3 sm:grid-cols-2"></div>
      </section>
...
const scene = new HomelabScene({
  ...
  onNodeSelect: (snapshot) => setSelection(snapshot, { focus: true }),
});
...
async function fetchMetrics() {
  ...
  if (!currentSelection) {
    const monitoringNode = metrics.nodes.find((node) => node.id === 'aqn-node1') ?? metrics.nodes[0];
    setSelection(monitoringNode, { focus: true });
    return;
  }
